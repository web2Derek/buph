$(document).ready(function() {
    ...
    var id_list = $('#memberlist_id').DataTable({
         "processing": true, //Feature control the processing indicator.
         "serverSide": true, //Feature control DataTables' server-side processing mode.
         "order": [[0,'desc']], //Initial no order.
         "columns":[
              {"data" : "acount_id" , render : function(data, type,row,meta) {
                var str = `<input type="checkbox" class="addtolist" remove_id ="${row.acount_id}">`;
                return str;
            }},
              {"data":"first_name"},
              {"data":"acount_id"},
    ...
    var table_branch = $('#tbl_branch').DataTable({
         ...
         "columns":[
              ...
              {"data":"branch_id","render": function(data, type, row,meta){
                        var str = '';
                        str += `
                         <a href="javascript:;" title="Edit Branch">
                         ...
                         <a href="javascript:;" title="Activate/Deactivate">
        ...
        $.ajax({
          ...
          success: function(data) {
            let errors = JSON.parse(data);
              if(errors.error) {
                let err = Object.keys(errors.error);
                $(err).each(function(idx, val) {
                $('input[name="'+val+'"]').next('.err-group').text(errors.error[val]);
                })
              } else {
                Swal.fire(
                  'Success',
          ...
          error: function(xhr, ajaxOptions, thrownError) {
            ...
            // alert(thrownError);
            Swal.fire('Error', 'Server Error Occur', 'error');
          }
        })
       ...
       $.ajax({
          ...
          success:  function(response) {
            ...
            error: function(response, status) {
              Swal.fire('Error', 'No Phone Connected', 'error');
          }
        })
      })
    })
...
//===================================================== UPDATE BRANCH
            ...
            $.ajax({
                ...
                success : function(data) {
                      if (data.form_error) {
                        ...
                        Swal.fire("Error in Server.",data.error, "error");
                    }
                },
            })
        });
    ...
    function generate_qr(idx, data_id) {
        let qr_code = $('#qr-code'+data_id)[0];
      ...
      // ============== SAVE QR CODE IN DB
              function save_qr(qr_id) {
                ...
                $.ajax({
                  ...
                  success: function(data) {
                    // console.log(`Generated QR for ${mem_id}`);
                    // Swal.fire('Success', 'Images Save', 'success');
...
// $('.profile_new').on('click' , function() {
//   let element = $(this);
//   Swal.fire({
//     title:'Profile',
//     html:`
//       <div class="btn-group btn-group-lg">
//       <button type="button" class="btn waves-effect waves-light btn-outline-warning btn-sm btn_g" id="upload_new">Upload</button>
//       <button type="button" class="btn waves-effect waves-light btn-outline-warning btn-sm btn_g" id="take_picture_new">Take picture</button>
//       </div>
//     `,
//     showCloseButton : true,
//     showConfirmButton : false
...
$('.profile_edit').on('change' , function() {
    ...
    if (checkValidation != -1) {
        ...
        Swal.fire('Erro','Invalid file format.', 'error');
    }
...
// $(document).on('click' , '#upload_new' , function() {
//     let element = $('.profile_new');
//     $(element).attr('type' , 'file');
//     $(element).click();
//     Swal.close();
// });
...
// $(document).on('click' , '#take_picture_new' , function() {
//     $('#controls').show();
//     Webcam.set({
//       width: 208,
//       height: 190,
